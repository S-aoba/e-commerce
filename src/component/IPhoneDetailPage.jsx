...
const IPhoneDetailPage = ({
  ...
  let iPhoneInfo = Methods.getIPhoneInfo(params.iphone);
  const [detailSubTotal, setDetailSubTotal] = useState(iPhoneInfo.price * num);

  useEffect(() => {
    // カートの個数が変化する度に、subTotalを更新する
    // subTotal部分⬇︎
    const newSubTotal = Number(num) * iPhoneInfo.price;
    setDetailSubTotal(newSubTotal);
  }, [num]);

  useEffect(() => {
    const hasF = favoriteList.indexOf(iPhoneInfo);
    if (hasF !== -1) setFavorite(true);
  }, []);

  useEffect(() => {
    const hasCart = cartList.indexOf(iPhoneInfo);
    if (hasCart !== -1) setCartFlag(true);
  }, []);

  const onFavorite = (
    <button
      onClick={() => Methods.isFavorite(iPhoneInfo, favoriteList, setFavorite, setFavoriteList)}
  ...
  const handleNumber = (e) => {
    ...
    if (val[0] === '0') {
      alert('正しい値を入力してください');
      setNum(1);
      setDetailSubTotal(iPhoneInfo.price);
    } else {
      setNum('');
      setNum(val);
    }
    ...
    <div className="mt-10 flex justify-center">
      <div className="w-7/12 mr-5 border-2 border-gray-300">
        <div className="flex justify-center">
          <div className="my-10">
            <img className="block w-96" src={iPhoneInfo.imgURL} alt="" />
            <div className="mt-10 flex justify-center">
              <p className="block text-5xl">{iPhoneInfo.productName}</p>
            </div>
          </div>
        </div>
        <div className="my-5 border-2 border-gray-200 mx-20 flex justify-center">
          <p className="px-3 py-3 text-5xl font-serif">{iPhoneInfo.description}</p>
        </div>
      </div>
      <div className="w-96 border-2 border-gray-300 flex justify-center">
        <div className="mt-16">
          <div className="my-3">
            <p className="text-3xl ">商品名: {iPhoneInfo.productName}</p>
          </div>
          <div className="flex justify-end">
            <div>
              <p className="my-5 text-3xl">価格: {iPhoneInfo.price}円</p>
